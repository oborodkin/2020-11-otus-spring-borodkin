...
public class QuestionTest {
    ...
    @DisplayName("Сверяет правильный ответ")
    @Test
    void shouldCheckAnswerTrue() {
        List<String> tstAnswers = new ArrayList<>();
        tstAnswers.add("abc");
        tstAnswers.add("qwerty");
        Question question = new Question(1, "Пример вопроса", "input", tstAnswers, "1");
        question.setUserAnswer("1");
        assertThat(question.checkAnswer()).isTrue();
    }

    @DisplayName("Сверяет неправильный ответ")
    @Test
    void shouldCheckAnswerFalse() {
        List<String> tstAnswers = new ArrayList<>();
        tstAnswers.add("abc");
        tstAnswers.add("qwerty");
        Question question = new Question(1, "Пример вопроса", "input", tstAnswers, "1");
        question.setUserAnswer("F");
        assertThat(question.checkAnswer()).isFalse();
    }
